...
function Commentt() {
  ...
  // 導去編輯評論
  const goToEdit = (item) => {
    history.push(`/member-center/commentEdit/${item.review_sid}`)
  }

  async function getCommentt() {
    const response = await request({
      url: `/members/getCommentt`,
      method: 'POST',
    })
                  ...
                  <div>
                    ...
                    <div style={{ marginTop: '15px' }}>
                      <Image
                        ...
                        alt="img"
                        onError={(e) => {
                          e.target.style.width = '150px' // 加載錯誤重新設定圖片大小(太大了)
                          e.target.onerror = null
                        }}
                        // src={`${window.location.origin}/img/${item.photo}`}
                        src={`${
                          item.photo.startsWith('http')
                            ? item.photo
                            : `${window.location.origin}/img/${item.photo}`
                        }`}
